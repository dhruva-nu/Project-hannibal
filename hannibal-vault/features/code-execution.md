...
**What the student sees.** Ordinary connection strings on ordinary hostnames, exactly like a real deployment — `ECHO_URL=tcp://echo:7777`, `DATABASE_URL=postgresql://…@db:5432/app`, `REDIS_URL=redis://cache:6379`, `MONGO_URL=mongodb://db:27017/app`, `AMQP_URL=amqp://…@queue:5672/`. The hostnames are network aliases on the emulator container. `echo` and `redis` have backing emulators today ([cannae-service](../../cannae-service/README.md)); `postgres`, `mongo`, and `amqp` are the locked contract Phases 2–4 fill in. A caching lesson declares `infra: ["redis"]` and the student writes ordinary `redis://cache:6379` code against a RESP2 emulator whose clock, expiry, and stale reads the harness scripts — see [Phase 1](../../plans/infra-emulators.md#3-phase-1--cache-redis-resp2--shipped-134-cratescannae-cache). Lesson-declared env is applied **before** the deps provider's, so nothing can shadow `PYTHONPATH` / `NODE_PATH`.
