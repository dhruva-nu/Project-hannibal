...
func TestDevServesTheDashboardWithNoChild(t *testing.T) {
	...
	code := serveDashboard(
		devOptions{bind: "127.0.0.1:0", configPath: writeConfig(t, redisOnly)},
		&stdout, &stderr,
		func(url string) {
			state := poll(t, url)
			...
			if len(state.About.Services) != 1 || state.About.Services[0] != "redis" {
...
func TestDevArmsTheConfigsFaultsBeforeAnyoneConnects(t *testing.T) {
	config := writeConfig(t, `{"services":["redis"],"faults":[{"match":"redis.SET","action":"error"}]}`)
	var stdout, stderr bytes.Buffer
...
func TestDevReportsAnUnusableConfig(t *testing.T) {
	for name, path := range map[string]string{
		...
		"a fault that could never fire": writeConfig(t, `{"services":["redis"],"faults":[{"match":"redis.*","action":"cap"}]}`),
	} {
...
func TestRunServesTheDashboardBesideTheChild(t *testing.T) {
	...
	go func() {
		finished <- Run([]string{
			"run", "--config", writeConfig(t, redisOnly), "--" + devControlBindFlag, "127.0.0.1:0", "--",
