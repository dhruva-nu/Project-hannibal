...
from app.services.rce import docker as rce_docker
from app.services.rce import installer, two_phase
from app.services.rce.config import LIMITS, RUNTIME
from app.services.rce.deps import DEPS_PROVIDERS
from app.services.rce.deps.cache import install_phase_mounts, run_phase_mounts
from app.services.rce.docker import run_code
from app.services.rce.install_queue import InstallQueue
from app.services.rce.installer import INSTALL_LIMITS, install_packages
...
class TestStudentCodeNeverOnline:
    async def test_installer_command_carries_no_student_code(self, mocker):
        ...
        installer_client = _mock_client(
            mocker, "app.services.rce.installer._get_client"
        )
    ...
    def test_the_container_that_runs_student_code_is_offline(self, mocker):
        run_client = _mock_client(mocker, "app.services.rce.docker._get_client")
...
class TestMountPosture:
    ...
    def test_installer_container_gets_no_docker_socket(self, mocker):
        client = _mock_client(mocker, "app.services.rce.installer._get_client")
...
class TestRunSandboxUnchanged:
    ...
    def test_full_lockdown_snapshot(self, mocker, language):
        client = _mock_client(mocker, "app.services.rce.docker._get_client")
