...
from app.services.rce.deps.cache import install_phase_mounts
from app.services.rce.deps.javascript import _npm_install_cmd
from app.services.rce.deps.python import _pip_install_cmd
from app.services.rce.installer import INSTALL_LIMITS, install_packages
from app.services.rce.prewarm import prewarm_all
...
def _mock_client(mocker, *, exit_code=0, wait_raises=None, stderr=b""):
    ...
    mocker.patch("app.services.rce.installer._get_client", return_value=client)
...
class TestPrewarm:
    def test_prewarm_installs_every_allowlist(self, mocker):
        installed = mocker.patch("app.services.rce.prewarm.install_packages")
